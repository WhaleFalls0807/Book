			...
			<tbody>
				...
				<c:forEach items="${books}" var="book">
					<tr>
						<td>${book.id }</td>
						<td>${book.name }</td>
						<td><img src="<%=request.getContextPath()%>/${book.cover}" width="20" height="20"
							onmouseenter="imgBig(this)" onmouseleave="imgSmall(this)" /></td>
						<td>${book.price }</td>
						<td>${book.category }</td>
						<td>
							<a class="layui-btn layui-btn-xs"  id="${book.id }" href="${pageContext.request.contextPath}/book/toUpdate?id=${book.id }">修改</a> <a
